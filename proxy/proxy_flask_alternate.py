...
def root(url):
    ...
    if proxy_ref:
        ...
        redirect_url = "/p/%s/%s%s" % (proxy_ref[0], url, ("?" + query_string if query_string else ""))
...
def proxy(url):
    ...
    return r.text  # Response(generate(), headers=headers)
...
def is_approved(url):
    ...
    return True
    # return host in APPROVED_HOSTS fixme tmp
